...
// Shared-data locking.
//
// g_addrMap and BLEPower::_devices are written from the NimBLE host task
// (scan results / callbacks) and read from the async_tcp task (web API) and
// the main loop task (connect logic).
//
// IMPORTANT: all three are FreeRTOS TASK contexts, never ISRs. The previous
// portENTER_CRITICAL/portEXIT_CRITICAL sections disabled interrupts/scheduling
// while std::map / std::vector performed heap allocations (operator[] insert,
// push_back, vector copy) - a classic source of rare deadlocks/crashes.
//
// A FreeRTOS mutex is the correct primitive here: it may legally block and
// heap allocation while holding it is allowed. Sections are kept short so the
// NimBLE host task is never delayed meaningfully.
// ---------------------------------------------------------------------------
static SemaphoreHandle_t g_dataMutex = nullptr;

static inline void dataLock()   { if (g_dataMutex) xSemaphoreTake(g_dataMutex, portMAX_DELAY); }
static inline void dataUnlock() { if (g_dataMutex) xSemaphoreGive(g_dataMutex); }
...
class ScanCallbacks : public NimBLEAdvertisedDeviceCallbacks {
  void onResult(NimBLEAdvertisedDevice *dev) override {
    ...
    NimBLEAddress bleAddr = dev->getAddress();   // copy made OUTSIDE the lock
    dataLock();
    g_addrMap[addr] = bleAddr;
    dataUnlock();
...
void BLEPower::begin() {
  if (!g_dataMutex) g_dataMutex = xSemaphoreCreateMutex();
  NimBLEDevice::init("");
...
std::vector<BLEDeviceInfo> BLEPower::getDevices() {
  dataLock();
  std::vector<BLEDeviceInfo> copy = _devices;   // single heap-allocating copy, then release
  dataUnlock();
  return copy;
}
