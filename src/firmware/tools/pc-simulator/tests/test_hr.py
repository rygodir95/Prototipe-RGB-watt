...
class TestHrZones(unittest.TestCase):
    def test_defaults_generated_from_hr_max(self):
        c = fw.AppConfig()
        self.assertEqual(c.hr_max, 0)                 # 0 = "not set" until configured
        self.assertEqual([z.min_bpm for z in c.hr_zones], [0, 1, 2, 3, 4])  # trivial
    ...
    def test_hr_color_interpolation(self):
        ...
        self.assertEqual((r, g, b), (c.hr_zones[4].r, c.hr_zones[4].g,
                                    c.hr_zones[4].b))            # Z5 Maximum, solid
        r, g, b = fw.hr_color_for(c, 50)
        self.assertEqual((r, g, b), (c.hr_zones[0].r, c.hr_zones[0].g,
                                    c.hr_zones[0].b))           # below first: Z1
