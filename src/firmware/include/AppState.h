...
// until the host has processed the DISCONNECT event. The Hub has a bounded
// connection budget, so setControlSource() registers the old module's link
// probe here when its link was still active; the new source's one-shot
// connect executor
